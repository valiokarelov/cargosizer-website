...
const CarrierLinks = () => {
  ...
  const getAirlineCode = (name) => {
    const match = name.match(/^\d{3}\s+([A-Z0-9]{1,3})/)
    return match ? match[1] : ''
  }
    ...
    <div className="carrier-links-container">
      <div className="carrier-links-header">
        ...
        <p className="carrier-links-description">
          Direct access to {airlines.length} airline cargo tracking websites. Click any airline to visit their tracking page.
        </p>
      </div>
          ...
          >
            <div className="airline-header">
              <div className="airline-codes">
                <span className="awb-prefix">{getAWBPrefix(airline.name)}</span>
                <span className="airline-code">{getAirlineCode(airline.name)}</span>
              </div>
              <ExternalLink className="external-icon" />
            </div>
            
            <div className="airline-name">
              {getCleanName(airline.name)}
            </div>
            
            <div className="airline-url">
              {airline.url.replace('https://', '').replace('http://', '').split('/')[0]}
            </div>
          </a>
        ))}
      </div>
